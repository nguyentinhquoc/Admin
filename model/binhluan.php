...
function load_binhluan(){
   $sql = "SELECT binhluan.trangthai,binhluan.id,taikhoan.name 'nameuser',binhluan.comment,sanpham.name,binhluan.date 
   FROM `taikhoan` 
   JOIN binhluan on binhluan.iduser=taikhoan.id 
   JOIN sanpham on binhluan.idsp = sanpham.id 
   ORDER BY binhluan.date DESC";
   $load_binhluan = pdo_query($sql);
   return $load_binhluan;
